# AGENTS.md — working in ezmodel

**ezmodel** is driven by [pyclawd](https://github.com/julian/pyclawd), a
...
- **Out of scope for the gate**: `ezmodel/experimental/` (rough, partially-broken
  scratch code with dead imports) and `tests/scratch.py`. Excluded in ruff
  (`extend-exclude`), mypy (`exclude`), and `DescriptionConfig.exclude` — keep the
  three aligned.
- **Optional model backends** (`GPy`, `gpflow`, `smt`, `pySOT`) are not installed by
  default. The usage-script test (`tests/test_usage.py::test_usages`) needs them and is
  marked `@pytest.mark.optional`, excluded from the default test tier. Run it with the
  `all` tier after installing those backends.
- **Behavior oracle**: `pyclawd golden` (baselines in `tests/golden/`) plus the unit
  suite. Adoption edits are mechanical — they must not move any number.
...
pyclawd python -c "import ezmodel" # quick check
```
